...
public class LogicofOrder{
	...
	public void addToViewOrder(String order) {
		...
		viewOrder.append(System.getProperty("line.seperator"));
	}
	...
	public double getTotalDiscount(int quantity, double itemPrice) {
		...
		if ((quantity >= 5) && (quantity <=9))
			return 0.9 * (quantity * itemPrice);
		//15
		if ((quantity >= 10) && (quantity <=14))
			return 0.85 * (quantity * itemPrice);
		//20
		if (quantity >= 15)
			return 0.80 * (quantity * itemPrice);
